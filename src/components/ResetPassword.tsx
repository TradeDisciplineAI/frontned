...
export const ResetPassword: React.FC<ResetPasswordProps> = ({ onBackToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.detail?.[0]?.msg || "Failed to reset password. The link may have expired.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="auth-form-card">
        ...
        {isSuccess ? (
          ...
          <form onSubmit={handleSubmit} className="auth-form" noValidate>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.8rem', color: 'var(--color-text-grey)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                {hasMinLength ? <CheckCircle2 size={14} color="var(--color-brand-teal)" /> : <div style={{ width: '12px', height: '12px', borderRadius: '50%', border: '1px solid #cbd5e1', marginLeft: '1px' }} />}
                <span style={{ color: hasMinLength ? 'var(--color-text-dark)' : 'inherit' }}>At least 12 characters long</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                {hasNumberOrSymbol ? <CheckCircle2 size={14} color="var(--color-brand-teal)" /> : <div style={{ width: '12px', height: '12px', borderRadius: '50%', border: '1px solid #cbd5e1', marginLeft: '1px' }} />}
                <span style={{ color: hasNumberOrSymbol ? 'var(--color-text-dark)' : 'inherit' }}>Includes a number or symbol</span>
              </div>
            </div>
